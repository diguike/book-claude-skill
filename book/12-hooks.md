---
title: Hooks：Skill 的行为钩子
feishu_url: "https://fivwvysqdz.feishu.cn/docx/MgcTdwkSsoFjQpxxEaccVLPQnzM"
...
Hooks 可以配在两个地方，这是第一版书里没讲清的细节：

### 全局/项目级 hooks：`.claude/settings.json`

这种 hook 不绑定到具体 Skill，对所有匹配 `matcher` 的事件都触发。例如"任何时候 Write 文件后跑 prettier"。
...
### Skill 级 hooks：frontmatter 的 `hooks` 字段

这种 hook **只在该 Skill 的生命周期内生效**。Skill 没被触发时，hook 不挂载；Skill 结束后 hook 卸载。例如"当 code-review Skill 在工作时，PostToolUse 跑指标收集"。
...
hooks:
  PostToolUse:
    - matcher: "Skill:code-review"
      hooks:
        - type: command
          command: "npx tsx ${CLAUDE_SKILL_DIR}/scripts/collect-metrics.ts"
---
```
...
| 场景 | 选哪个 |
|------|--------|
| 通用规则（"任何代码改动都跑 prettier"） | `.claude/settings.json` |
| 跟着 Skill 走的处理（"这个 Skill 跑完收集指标"） | frontmatter `hooks` |
| 跨 Skill 共享的安全检查 | `.claude/settings.json` |
| 装在 plugin 里跟着分发 | plugin 内 `hooks/hooks.json`（见第 25 章） |

第一版书把这件事写糊了——大部分例子放 `.claude/settings.json`，但第 7 章 frontmatter 表格又有 `hooks` 字段。两者**是同一套机制的不同挂载位置**，frontmatter 是 Skill 局部声明，settings 是全局声明。Plugin 里的 `hooks/hooks.json` 是第三种入口，本质和 settings 一样但跟着 plugin 走。

## 配置方式（项目级别）

把 hook 写到 `.claude/settings.json`，提交到仓库后所有人共享：
...
- **v6**（第 10 章）：加入脚本，AI 负责调用
- **v7**（第 11 章）：加入自评分和趋势追踪，开始用数据衡量质量
- **v8**（第 12 章）：后处理从指令移到 hook，执行确定性从"大概率"变成"必定"
