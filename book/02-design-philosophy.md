---
title: 第 2 章 Skill 的设计哲学
feishu_url: "https://fivwvysqdz.feishu.cn/docx/WQbZdRGfxo4pA8x16RXcM0Pln8e"
...
这六条设计原则来自 Anthropic 团队的工程经验，但它们落到的载体——SKILL.md 文件格式、frontmatter 字段、目录约定——是一个**开放规范**。

[Agent Skills](https://agentskills.io) 由 Anthropic 发起、开放给社区共建。规范文档在 GitHub 上维护（[agentskills/agentskills](https://github.com/agentskills/agentskills)），任何人都能提 PR 改进规范。目前已经被 30+ 主流 AI 客户端支持，包括 Claude、Claude Code、Cursor、Gemini CLI、GitHub Copilot、VS Code、JetBrains（Junie）、OpenAI Codex 等。

这件事对工程决策的影响很关键。如果你担心"投入写一堆 Skill 之后某天平台跑路怎么办"——这是写 Skill 之前真实存在的顾虑，规范本身的开放性是最有力的回应。你写的 SKILL.md 是个开放格式的文本文件，能在多个客户端运行，跨工具迁移成本极低。Anthropic 自己的态度也很明确——这套规范是开源捐赠出来的，不是平台锁定的工具。

但有一点要分清楚：规范定义了**核心字段**（`name`、`description`、`license`、`compatibility`、`metadata`、`allowed-tools`），但各平台会扩展自己的字段。Claude Code 特有的 `context: fork`、`agent`、`hooks`、`paths`、`shell`、`when_to_use`、`disable-model-invocation` 等不在规范里——它们是 Claude Code 的平台扩展。

工程上的纪律：

- **核心逻辑只依赖规范字段**：这样跨平台迁移时不用大改
- **平台扩展字段加注释说明**：在 frontmatter 里写 `# Claude Code specific`，让其他平台用户一眼看出
- **持续关注规范更新**：[agentskills.io/specification](https://agentskills.io/specification) 是规范的权威文档。新字段加入规范后，先看再用
