---
title: 第 28 章 毕业项目：从零构建 changelog-generator
feishu_url: "https://fivwvysqdz.feishu.cn/docx/FOoadTWf5oh4HwxQc4mc5Yd1nWd"
...
| 需求 | 对应章节 | 具体知识点 |
|------|---------|-----------|
| 读取 git log 注入上下文 | 第 7 章 动态上下文 | `!`command`` 语法 |
| Markdown 输出格式 | 第 6 章 写好 Skill 指令 | 模板 + 示例驱动 |
| 不同项目使用不同分类规则 | 第 9 章 可插拔规则 | references/ 外挂规则文件 |
| TypeScript 脚本解析 git log | 第 10 章 在 Skill 中嵌入脚本 | scripts/ 目录使用 |
| 评估分类准确率 | 第 11 章 让 Skill 自我记录 | 内置评分逻辑 |
| 生成后自动 commit | 第 12 章 Hooks | hooks 配置 |
| 测试用例和断言 | 第 19 章 让评测真正有用 | evals.json 编写 |
...
完成后可以对照 `examples/ch28-graduation/changelog-generator/` 中的参考实现。但先自己做，做完再看。参考答案只是一种实现方式，不是标准答案。
