---
title: 解剖 skill-creator：创建 Skill 的 Skill
feishu_url: "https://fivwvysqdz.feishu.cn/docx/TPVQd5qbcoIW6ExgQM4cJwSJnq6"
last_synced: "2026-06-03T14:13:32Z"
---


> 这一章深入拆解了 skill-creator 的内部架构。如果你更想先动手写 Skill，可以跳到第 4 章——那时你会对 SKILL.md 结构有切身体验，回头读这里理解起来更顺畅。
...
grader 内部有一个 7 步评测流程，从读 transcript、逐条评判断言、到批判断言质量本身，形成完整的双层检验。第 19 章已经展开了 7 步的具体内容，这里讲它在 skill-creator 内部的工程实现。

## 盲比系统：comparator 的设计

改进了一个 Skill 之后，怎么判断是不是真的变好了？直接对比有一个根本问题：你知道哪个是新版本，确认偏差会让你倾向于觉得改过的更好。

comparator 用盲比解决这个问题——两个输出匿名呈现，顺序随机，评完分之后才揭示来源。评分分"内容"和"结构"两个维度，防止出现"正确性提高但格式变差"的此消彼长。第 19 章已经展开了评分维度和判定标准，这里讲它在 skill-creator 内部如何实现匿名化和顺序随机。
